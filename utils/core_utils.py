...
def train(datasets, cur, args):
    ...
    # Init model
    if args.model_type in ['clam_sb', 'clam_mb', 'hit', 'dsmil', 'pam', 'transmil', 'dtfdmil', 's4']:
        if args.subtyping:
        ...
        elif args.model_type == 'pam':
            model = PAM(**model_dict)
        else:
    ...
    for epoch in range(args.max_epochs):
        ...
        elif args.model_type in ['hit', 'dsmil', 'hipt_lgp', 'hipt_n', 'pam', 'transmil', 'dtfdmil', 's4'] and not args.no_inst_cluster:     
    ...
    if args.model_type in ['hit', 'dsmil', 'hipt_lgp', 'hipt_n', 'pam', 'transmil', 'dtfdmil', 's4']:
...
def train_loop_hit(epoch, model, loader, optimizer, n_classes, bag_weight, writer = None, loss_fn = None, model_type='None'):
    device=torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model.train()
    acc_logger = Accuracy_Logger(n_classes=n_classes)
    ...
    for batch_idx, (data, data2, label, slide_id, coords) in enumerate(loader):
        ...
        logits0, Y_hat, Y_prob, _ = model(data, data2)
    ...
    print('Epoch: {}, train_loss: {:.4f}, trans_loss2: {:.4f}, train_error: {:.4f}'.format(epoch, train_loss, train_loss2,  train_error))
    acc_list = []
    correct_list = []
    count_list = []
    for i in range(n_classes):
